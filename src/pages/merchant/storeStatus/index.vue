<template>
  <div id="status">
    <mp-toast :merchandise="merchandise" :display="display" @close="closeStockPanel" @modifyStock="modifyStock"/>
    ...
    <div id="tab_content">
      <div class="tab_content_item store-orders" v-if="cur === 0">
        <store @showToast="showToastFunction" :onLoadCategory="onLoadCategory" :changeCategory="changeCategory"/>
      </div>
      <div class="tab_content_item purchase-orders" v-if="cur === 1">
        <sales :salesInfo="salesInfo" :sellTop="sellTop" :merchandiseTop="merchandiseTop" :statics="statics"
               :mySalesEChart="mySalesEChart" :onLoadSales="onLoadSales" :onLoadSalesEChart="onLoadSalesEChart"/>
      </div>
      <div class="tab_content_item sales-orders" v-if="cur === 2">
        <purchase :onLoadPurchase="onLoadPurchase"/>
      </div>
    </div>
