<template>
	<div class="body">
		...
		<div class="goods">
			...
			<m-list :height="listHeight" :width="listwidth" model="" :next="next" :list="merchandises" :addMerchandiseToCart="addCart"
			 :reduceMerchandiseToCart="reduceCart" :categoryId="categoryId"></m-list>
		</div>
		<cart v-if="isShowCart" @hdlShowPopup="hdlShowPopup" :emptyMerchandiseCart="emptyCart" :addMerchandiseToCart="addCart"
		 :reduceMerchandiseToCart="reduceCart"></cart>
	...
	export default {
		...
		methods: {
			...
			menusChange: function(index) {
				...
				this.$store.dispatch('model.reserveShop.merchandises/setCurrentCategory', {
					categoryIndex: index
				});
				this.loadMerchandises(1);
			},
